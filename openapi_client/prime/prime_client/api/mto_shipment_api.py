...
class MtoShipmentApi(object):
    ...
    def __init__(self, api_client=None):
        ...
        self.update_shipment_destination_address_endpoint = _Endpoint(
            settings={
                'response_type': (ShipmentAddressUpdate,),
                'auth': [],
                'endpoint_path': '/mto-shipments/{mtoShipmentID}/shipment-address-updates',
                'operation_id': 'update_shipment_destination_address',
                'http_method': 'POST',
                'servers': None,
            },
            params_map={
    ...
    def update_shipment_destination_address(
        ...
        **kwargs
        ...
        ### Functionality This endpoint is used so the Prime can request an **update** for the destination address on an MTO Shipment, after the destination address has already been approved. If automatically approved or TOO approves, this will update the final destination address values for destination SIT service items to be the same as the changed destination address that was approved. Address updates will be automatically approved unless they change:   - The service area   - Mileage bracket for direct delivery   - the address and the distance between the old and new address is > 50   - Domestic Short Haul to Domestic Line Haul or vice versa       - Shipments that start and end in one ZIP3 use Short Haul pricing       - Shipments that start and end in different ZIP3s use Line Haul pricing  For those, changes will require TOO approval.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True

        >>> thread = api.update_shipment_destination_address(mto_shipment_id, if_match, body, async_req=True)
        >>> result = thread.get()
