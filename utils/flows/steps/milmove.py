...
def do_hhg_create_move_with_multiple_shipments(
    flow_context: FlowContext,
    flow_session_manager: FlowSessionManager,
) -> None:
    """
    Creates a move with multiple shipments. Can raise internal_client.ApiException
    """
    api_client = flow_session_manager.internal_api_client(UserType.MILMOVE)
    ...
    mto_shipment_api_client.create_mto_shipment(
        body=CreateShipment(
            move_task_order_id=move_id,
            shipment_type=MTOShipmentType("HHG"),
            requested_pickup_date=(issue_date + datetime.timedelta(days=2)).date(),
            requested_delivery_date=(issue_date + datetime.timedelta(days=8)).date(),
            pickup_address=residential_address,
        )
    )
